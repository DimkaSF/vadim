            ...
            $.get(
                ...
                function(data){
                    ...
                    for(var i = 0; i < data.length; i++){
                        if(data[i]['ph_name'].indexOf("cover") > 0){
                            $("<div class='edit_al_photo_preview'>Текущая обложка:</div>")
                                .append("<img src='/img/"+data[i]["al_name"]+"/"+data[i]["ph_name"]+"' width='100%'>")
                                .append("<button onclick=\"edit_new_cover()\">Новая обложка</button>")
                                .appendTo("#edit_al_cover");
                        }
                        else{
                            $("<div class='edit_al_photo_preview'></div>")
                                .append("<img src='/img/"+data[i]["al_name"]+"/"+data[i]["ph_name"]+"' data-al="+data[i]["al_id"]+" data-ph="+data[i]["ph_id"]+" width='150px'>")
                                .append("<div class='delete_one_photo'><div onClick='delete_one_photo($(this))' data-al="+data[i]["al_id"]+" data-ph="+data[i]["ph_id"]+">Удалить</div></div>")
